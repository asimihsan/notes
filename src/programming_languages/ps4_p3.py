...
def csuffix(X,Y):
    ...
    if len(X) > 0 and len(Y) > 0 and X[-1] == Y[-1]:
        return_value = 1 + csuffix(X[:-1], Y[:-1])
    else:
        return_value = 0
    chart[(X,Y)] = return_value
    return return_value
...
def lsubstring(X,Y):
    return max(
        csuffix(prefix_x, prefix_y)
        for prefix_x in prefixes(X)
        for prefix_y in prefixes(Y)
    )

# We have included some test cases. You will likely want to write your own.
print '-' * 20 + ' csuffix ' + '-' * 20
print csuffix("abcde", "abcde") == 5
print csuffix("", "") == 0
print csuffix("abcde", "edcba") == 0
print csuffix("abcde", "abcdd") == 0
print csuffix("cccde", "abcde") == 3
print '-' * 20 + ' csuffix ' + '-' * 20
