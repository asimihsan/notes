...
def gruesome(input_string):
    matches = re.finditer(r'[a-zA-Z]+', input_string)
    strings = (match.group(0) for match in matches)
    re_grue = re.compile(r'b.*t|t.*g')
    gruesome_words = (word for word in strings if re_grue.search(word))
    return [word for word in gruesome_words]
